...
public:
    RedFlower(float x, float y) {
        position.x = x;
        position.y = y;

        Animation& normal_animation = animation_pool["normal"];
        normal_animation.add_frame(ResourcesManager::instance()->find_atlas("red_flower"));
        ...
        image = ResourcesManager::instance()->find_atlas("red_flower")->get_image(0);
    }

    void on_render(const Camera& camera) override{
        if (!current_animation) return;

        render_shade(camera, position.x - 8, position.y + 4,
