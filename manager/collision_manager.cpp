    ...
    for(CollisionBox* collision_box_src : collision_box_list) {
        ...
        for (CollisionBox * collision_box_dst : collision_box_list) {
            ...
            if (!collision_box_dst) {
                std::cerr << "collision_box_dst is nullptr" << std::endl;
                continue;
            }

 

            if (collision_box_dst == collision_box_src  ||  // 如果碰撞盒为自己或目标层为None，则跳过
                !collision_box_dst->enabled // 如果碰撞盒未启用，则跳过
                || collision_box_src->layer_dst != collision_box_dst->layer_src) continue; // 如果碰撞盒未启用或目标层不匹配，则跳过
            ...
            if (is_collide_x && is_collide_y) {
                ...
                if( collision_box_src->on_collide)
                    collision_box_src->on_collide(); // 如果发生碰撞且源碰撞盒有处理函数，则调用处理函数
            }
        }
    }
}
...
void CollisionManager::on_debug_render(const Camera& camera) {
    const Vector2& pos_camera = camera.get_position(); // 获取相机位置

    for(CollisionBox* collision_box : collision_box_list) {
        // 绘制碰撞盒的矩形框
        setlinecolor(collision_box->enabled ? RGB(255, 195, 195) : RGB(115, 115, 175));
        rectangle(collision_box->position.x - collision_box->size.x / 2 - pos_camera.x,
            collision_box->position.y - collision_box->size.y / 2 - pos_camera.y,
            collision_box->position.x + collision_box->size.x / 2 - pos_camera.x,
            collision_box->position.y + collision_box->size.y / 2 - pos_camera.y);
